...
function getDimensions() {
  ...
  // svg size
  state.width = window.innerWidth - 16;
  state.height = (window.innerHeight - controlsHeight - headerHeight - 5 * 8) / _config.config.ranges.length;

  // wrapper, and elements size
  state.offsetWidth = state.width - state.margin.left - state.margin.right;
  state.offsetHeight = state.height - state.margin.top - state.margin.bottom;
}
...
function tick(ranges, datum) {
  ...
  datum.forEach(function (data, i) {
    ...
    if (data.length > _config.config.valuesCount) {
      data.shift();
      state.xExtent = d3.extent(datum[0], function (d, i) {
        return new Date(d.date);
      });
    } else {
      state.xExtent[1] = new Date(newValue.date);
    }

    // Cause set attr will be set after .xScale.domain
    setTimeout(function () {
  ...
  // new syntax ease('linear') ↦ ease(d3.easeLinear)
  state.xAxis[datum.length - 1].transition().duration(500).ease(d3.easeLinear).call(d3.axisBottom().scale(state.xScale));
}
...
function render(datum) {
  ...
  datum.forEach(function (data, i) {
    ...
    state.info[i].select('.title').attr('x', state.offsetWidth - 200 + state.margin.left).attr('y', state.margin.top - 14).attr('font-size', '18');
...
var _sockets = __webpack_require__(33);
...
function init(datum) {
  ...
  state.margin = {
    left: _config.config.margin.left,
    right: _config.config.margin.right,
    top: _config.config.margin.top,
    bottom: _config.config.margin.bottom
  };
  ...
  // new syntax d3.time.scale ↦ d3.scaleTime
  state.xScale = d3.scaleTime().domain(state.xExtent);
  ...
  datum.forEach(function (data, i) {
    ...
    state.line.push(d3.line()
    //  .curve(d3.curveBasis)
    .x(function (d) {
      return state.xScale(new Date(d.date));
    }).y(function (d) {
      return state.yScale[i](d.value);
    }));

    state.area.push(d3.area());

    state.wrapper.push(state.svg[i].append('g'));

    state.pathArea.push(state.wrapper[i].append('path').datum(data).attr('class', 'area').attr('transform', 'translate(' + (state.margin.left + 1) + ', 0)').attr('style', 'fill: ' + _config.config.colors[i].fill + ';'));

    state.pathLine.push(state.wrapper[i].append('path').datum(data).attr('class', 'line').attr('transform', 'translate(' + (state.margin.left + 1) + ', 0)').attr('style', 'stroke: ' + _config.config.colors[i].stroke + ';'));

    state.yAxis.push(state.wrapper[i].append('g').attr('class', 'axis axis--y'));

    state.xAxis.push(state.wrapper[i].append('g').attr('class', 'axis axis--x'));

    if (i !== datum.length - 1) {
      state.xAxis[i] = state.xAxis[i].append('rect').attr('height', 1).attr('fill', '#000');
    }

    state.info.push(state.wrapper[i].append('g'));

    state.info[i].append('text').attr('class', 'title');

    _config.config.texts[i].split('\n').forEach(function (text) {
      state.info[i].select('.title').append('tspan').text(text);
    });

    var value = state.info[i].append('text').attr('class', 'value');

    value.append('tspan').attr('class', 'number');
    value.append('tspan').attr('class', 'unit');
  });
...
var data = genData(_config.config.ranges);
(0, _init.init)(data);
